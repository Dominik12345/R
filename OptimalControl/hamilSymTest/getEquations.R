...
extractModel <- function(model) {
  ...
  matches <- grepl("d*[x,y]\\[*[1-9]\\]*",dpTestModel) #finds the equations of the ode
  eq <- dpTestModel[matches] 
  listBoo <- grepl("list",eq)
  return(eq[!listBoo]) #return equation without a list statement if present
}
...
formatModelEq <- function(modelEq) {
    ...
    if(length(trimDxy)>1) {
      order <- as.integer(substr(x = trimDxy, start = 1, stop = 1)) #convert character of all first elements of the string vector into numbers

...
parameters.solve <- c(a = parameters[1], omega = parameters[2], Gamma = 1.1, peak = 10, t.centre = mean(t.data))
...
Lorenz <- function(t, state, parameters.solve) {
  with(as.list(c(state, parameters.solve)),{
    dx1 <- a * sin(omega * t) * cos(omega * t)
    dx2 <- x1 - x2
    dx3 <- x2 - x3 + Perturbation(t,x1,x2,x3,x4, Gamma, peak, t.centre)
    dx4 <- x3 -x4

...
parameters.solve <- c(a = parameters[1], omega = parameters[2], Gamma = 0.5, peak = 20, t.centre = mean(t.data))
...
Lorenz <- function(t, state, parameters.solve) {
  with(as.list(c(state, parameters.solve)),{
    dx1 <- a * sin(omega * t) * cos(omega * t) + Perturbation(t,x1,x2,x3,x4, Gamma, peak, t.centre)
    dx2 <- x1
    dx3 <- x2
    dx4 <- x3

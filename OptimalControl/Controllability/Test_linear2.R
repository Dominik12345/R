...
testModel <- function(t,x,parameters) {
  ...
  dx1 = a * sin(omega * t) * cos(omega * t) + x[3]
  dx2 = x[1] + x[4]
  dx3 = x[2]
  dx4 = x[1]
...
t.data <- seq(0, 3, length.out = 500)
parameters.solve <- c(a = parameters[1], omega = parameters[2], Gamma = 0.5, peak = 1, t.centre = mean(t.data))
state <- c( x1 = x0[1], x2 = x0[2] , x3 = x0[3], x4 = x0[4])

Perturbation <- function(t,x1,x2,x3,x4, Gamma, peak, t.centre) {
  #perturbation function
  perturbation.temp <- Gamma * (t-t.centre) + peak 
...
Lorenz <- function(t, state, parameters.solve) {
  with(as.list(c(state, parameters.solve)),{
    dx1 <- x3 + a * sin(omega * t) * cos(omega * t)
    dx2 <- x1 + x4 + Perturbation(t,x1,x2,x3,x4, Gamma, peak, t.centre)
    dx3 <- x2
    dx4 <- x1
...
for (i in 1:length(t.plot) ) {
  y.plot[i] <- Perturbation(t.plot[i], x1.plot[i], x2.plot[i], x3.plot[i], x4.plot[i], 
                            parameters.solve["Gamma"],parameters.solve["peak"], parameters.solve["t.centre"])
}

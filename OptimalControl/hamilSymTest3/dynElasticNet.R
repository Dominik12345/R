dynElasticNet <- function(alphaStep,x0,parameters,times,alpha1,alpha2,measData,STD,modelFunc,measFunc,odeObj,optW,origAUC) {
  ...
  for (i in 1:maxIter) {
    ...
    print(paste0('Iteration ',i,' J[w]=',round(J[[i+1]],8),'     change J[w]: ',round((1-abs(J[[i+1]]/J[[i]]))*100,2),' %'))
    #print('AUCs:')
  ...
  rmse <- function(measureTimes,input){
    ...
    return(sqrt(colSums((yhat - y)^2)/nrow(yhat)))
  }
